...
# Hotfix 22 – Shelter Proximity Matching (2026-09-17)

Live-testing-confirmed bug: a real shelter, "Government Degree College Mingora," exists in the checked-in `shelters.csv`, but a flood question with `village_name="Mingora"` returned "shelter data is currently unavailable" instead of finding and citing it. Investigated with real evidence (repository code, real CSV data, real distance calculations) before any fix was proposed, per user direction; the proposed fix (radius value, false-positive risk, scope, test plan) was itself reported and approved before implementation. Branch: `feature/shelter-matching-fix`, branched off `main`.

### Fixed

- **Root cause: no shelter-matching logic existed anywhere in the pipeline.** `ShelterTool`/`ShelterEvidenceMapper` always returned every configured shelter's bare name, unfiltered and unlabeled, regardless of the selected village — with no district, coordinate, or distance signal attached to any of them. The LLM had no way to judge which of ~50 anonymous names was relevant to the selected location, so it correctly (per this project's existing grounding rules) declined to cite any specific shelter rather than guess — this was not an exact-name-match rejecting near-misses; there was no matching step at all.
- **`ShelterEvidenceMapper.to_graph()` (`backend/app/graph/mappers/configured_data_mappers.py`) now computes real distance from the request's coordinates to every configured shelter**, reusing the existing, already-tested `distance_kilometers`/`Point` from `backend/app/gis/distance.py` (no new math). Filters to `Operational` shelters within a new `DEFAULT_SHELTER_MATCH_RADIUS_KM = 5.0` constant, returns them sorted nearest-first, and populates the previously-always-`None` `nearest_shelter`/`available_capacity` fields for the first time. **5km** is the standard rural/humanitarian evacuation-planning heuristic (~1 hour walk at ~5km/h), independently confirmed against the real dataset: sorting all 51 real shelters by distance from Mingora's real coordinates shows every same-tehsil (Babuzai) shelter within 3.53km, then a gap to 6.17km before the next tehsil's shelters begin — 5km sits cleanly inside that gap, capturing 24 of 51 shelters (47%, not "everything") and correctly excluding the 9 shelters 37km+ away in Bahrain/Khwazakhela.
- **Discovered mid-fix and fixed as a necessary companion gap: `status` (Operational/Closed) was silently dropped at the repository-projection boundary** (`backend/app/config/datasets.py`, `backend/app/data/repositories.py`). Several of the distance-closest rows are 2009-era "Relief Camp"/"IDP Camp" entries marked `Closed` — one sits at the exact same coordinates as the village centroid. Restoring `status` and filtering to `Operational` was necessary before radius filtering alone; without it, this fix would have started recommending a decade-closed relief camp as a real option.
- `ShelterNode` (`backend/app/graph/nodes/evidence_nodes.py`) now passes `state.user_request.coordinates` into the mapper. With no coordinates (e.g. a typed custom village name), behavior is unchanged — the pre-existing full-list fallback, deliberately not fixed here (separate, smaller gap, out of the approved scope).
- **Confirmed in scope, not assumed:** Situation Room's "Full Assessment" village selector (`village-explorer.tsx`) calls the exact same `/conversation` endpoint as the Agent page — verified in code (identical `postConversation`/`locationFields`/`ChatMessageBubble` usage, and the component's own docstring: "deliberately not a second, parallel AI path"). This fix therefore applies to both paths with zero additional frontend or backend changes. Situation Room's separate, lightweight "Village Snapshots" cards (`VillageSummaryService`, Hotfix 17) never touch shelter data by design and correctly stay untouched.

### Verified

- Full backend suite (excl. golden): **348 passed, 53 skipped, 8 subtests passed** — up from 339 by 9 new tests: a new `test_shelter_evidence_mapper.py` (4 synthetic unit tests for the distance/status logic — radius boundary, closed-shelter exclusion, no-origin fallback, nearest-selection — plus a 4-test regression suite run against the real, checked-in `shelters.csv` reproducing the exact Mingora scenario), and one new test in `test_repository_projection.py` confirming `status` now survives projection for a real `Operational` and a real `Closed` row.
- Two pre-existing test fixtures updated as a necessary, disclosed consequence (their shelter/request coordinates were geographically unrelated, which had never mattered before distance became meaningful to the result): `test_graph_container.py` (one fixture shelter's coordinates moved near its request coordinates) and the gated, real-API `test_conversation_golden_set.py` (`RUN_GOLDEN_SET=1`, not run this session — its `_request()` helper's placeholder coordinates were literally Peshawar's despite being commented "realistic Swat request"; now genuinely Mingora's, matching its own shelter fixture and every village_name used in that file).
- **Real dual-path live verification, each path confirmed independently rather than assumed from the shared-code-path finding:**
  - **Agent path**: real production `shelters.csv`, real decision agent, real OpenAI call. Today's real GloFAS snapshot shows only minor discharge for Mingora, which the pre-existing `route_after_gis` policy correctly skips shelter/village/GIS collection for (unrelated to this bug, confirmed by first reproducing it via direct API call) — so a synthetic MAJOR-severity forecast was substituted (the same technique this project's own golden tests already use) while keeping shelters, villages, the decision agent, and the LLM call fully real. Result: *"Several shelters are available nearby, including Govt College of Technology Swat, Swat Public School and College Mingora, and Sports Complex Mingora... Residents should be directed to these shelters immediately,"* citations `('shelter', 'Mingora', 'gis')`, `missing_evidence: ()` — replacing the originally-reported "shelter data is currently unavailable." "Government Degree College Mingora" (the exact shelter from the bug report) is present in the underlying grounded evidence set.
  - **Situation Room's Full Assessment**: live in Chrome against the real running frontend + backend, selected "Mingora (Swat)" from the actual village selector, clicked "Get full assessment" — confirmed it hits the identical `/conversation` endpoint and returns the identical response shape/honesty as the Agent page under today's real (minor-severity) forecast conditions, verifying the shared-backend-logic claim directly rather than assuming it from code alone.

### Notes

- Stayed within the user-approved minimal scope: the one condition set in advance (stop and report back if the fix touched significantly more files, changed a shared interface other features depend on, or risked an unrelated test) was checked against explicitly — Situation Room's lightweight Village Snapshots endpoint was confirmed to need no changes, and the only test fallout beyond the new shelter-matching module itself was two disclosed, mechanical fixture-coordinate corrections in tests already about shelter grounding.
