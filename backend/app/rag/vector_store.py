...
class ChromaVectorStore(VectorStore):
    ...
    def _metadata(chunk: KnowledgeChunk) -> dict[str, object]:
        ...
        return {
            ...
            "agency": data.agency or "",
            "document_type": data.document_type,
            "publication_year": data.publication_year or 0,
            "page_number": data.page_number,
            "section": data.section or "",
            "heading": data.heading or "",
            "district": data.district or "",
            "river": data.river or "",
            "hazard_type": data.hazard_type or "",
    ...
    @staticmethod
    def _rehydrate(
        identifier: str, text: str, data: Mapping[str, object]
    ) -> KnowledgeChunk:
        return KnowledgeChunk(
            ...
            ChunkMetadata(
                ...
                agency=str(data.get("agency") or "") or None,
                document_type=str(data["document_type"]),
                publication_year=(
                    int(data["publication_year"])
                    if int(data.get("publication_year") or 0) > 0
                    else None
                ),
                page_number=int(data["page_number"]),
                section=str(data.get("section") or "") or None,
                heading=str(data.get("heading") or "") or None,
                district=str(data.get("district") or "") or None,
                river=str(data.get("river") or "") or None,
                hazard_type=str(data.get("hazard_type") or "") or None,
                keywords=tuple(
                    value
                    for value in str(data.get("keywords") or "").split(",")
                    if value
                ),
